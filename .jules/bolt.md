...
## 2024-07-29 - splitlines() vs io.StringIO boundary differences and memory premise
**Learning:** `splitlines()` and `io.StringIO(..., newline="")` are NOT drop-in replacements. `splitlines()` splits on a wider set of Unicode boundaries (like form feed `\x0c`, NEL `\x85`, U+2028, U+2029). Using `io.StringIO` silently re-segments texts with those characters, breaking deterministic block hashes. Additionally, replacing `splitlines()` with `io.StringIO(text)` on an already fully-materialized `str` yields NO peak memory win when the results are still collected into a list. For NDJSON, `text.split("\n")` is actually safer than `splitlines()` because JSON strings may legally contain U+2028/U+2029 characters.
**Action:** Do not substitute `splitlines()` with `io.StringIO()` for memory savings on strings that are already fully materialized into lists, especially if deterministic parsing over all Unicode line breaks is required.
